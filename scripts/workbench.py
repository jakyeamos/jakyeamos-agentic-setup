...
try:
    from .catalog_validation import load_manifest, validate_manifest
...
def _manifest_assets(root: Path) -> list[dict[str, Any]]:
    ...
    return sorted((asset for asset in assets if isinstance(asset, dict)), key=lambda item: item.get("id", ""))
...
def _search_text(asset: dict[str, Any]) -> str:
    fields: list[str] = []
    for key in ("id", "kind", "asset_class", "title", "summary", "maturity", "capabilities", "supported_targets"):
        value = asset.get(key)
...
def _dependency_report(asset: dict[str, Any]) -> tuple[list[dict[str, Any]], bool]:
    ...
    for dependency in asset.get("dependencies", []):
        ...
        available = shutil.which(command) is not None if isinstance(command, str) else None
        item = {
            "available": available,
            "command": command,
...
def _safe_destination(root: Path, relative: Path) -> Path:
    ...
    try:
        ...
        raise WorkbenchError(f"installation path escapes target root: {relative}") from exc
    return destination


def _install_plan(root: Path, asset: dict[str, Any], target: str, dry_run: bool) -> dict[str, Any]:
    ...
    if target not in supported_targets:
        raise WorkbenchError(f"asset {asset.get('id')} does not support target {target}")
    ...
    if mode in {"copy", "stage"}:
        destination_root = _safe_destination(root, Path(str(install.get("destination", ""))))
        for source in sorted(asset.get("files", [])):
            ...
            action_status = "exists" if destination.exists() else ("would-copy" if dry_run else "ready")
            actions.append(
                {
...
def _apply_install(root: Path, asset: dict[str, Any], plan: dict[str, Any]) -> dict[str, Any]:
...
def _parser() -> argparse.ArgumentParser:
    ...
    list_parser.add_argument("--json", action="store_true", help="emit deterministic JSON")

    search_parser = subparsers.add_parser("search", help="search catalog metadata")
    search_parser.add_argument("--query", required=True)
    search_parser.add_argument("--json", action="store_true", help="emit deterministic JSON")

    show_parser = subparsers.add_parser("show", help="show one catalog asset")
    show_parser.add_argument("asset_id")
    show_parser.add_argument("--json", action="store_true", help="emit deterministic JSON")

    install_parser = subparsers.add_parser("install", help="plan or apply a safe asset installation")
    ...
    install_mode.add_argument("--dry-run", action="store_true", help="preview only; this is the default")
    install_mode.add_argument("--apply", action="store_true", help="copy into the explicit target root")
    install_parser.add_argument("--json", action="store_true", help="emit deterministic JSON")

    validate_parser = subparsers.add_parser("validate", help="validate catalog, public safety, and skills")
    validate_parser.add_argument("--json", action="store_true", help="emit deterministic JSON")
    return parser
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        if args.command == "list":
            ...
            human = [f"{asset['id']}\t{asset['title']}\t{asset['asset_class']}" for asset in payload["assets"]]
        ...
        if args.command == "show":
            ...
            _emit(asset, args.json, [f"{asset['id']}: {asset['title']}", asset['summary']])
            return 0
        if args.command == "install":
            root = args.root.expanduser().resolve()
            if root == Path("/") or root == Path.home():
                raise WorkbenchError("refusing an installation root that is the filesystem or user home")
            ...
            if args.apply and plan["status"] == "ready":
                plan = _apply_install(root, asset, plan)
            human = [
        ...
        if args.command == "validate":
            ...
            payload = {"catalog": not any(error.startswith(("asset ", "catalog/", "source_map")) for error in errors), "errors": errors, "skills": not any(error.startswith(str(ROOT / "skills")) for error in errors), "safety": not any("private" in error or "credential" in error or "authorization" in error or "secret" in error or "environment file" in error for error in errors), "ok": not errors}
            if args.json:
